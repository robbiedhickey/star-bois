...
public sealed class PagerSystem : EntitySystem
{
    ...
    private void OnPacketReceived(EntityUid uid, PagerReceiverComponent component, DeviceNetworkPacketEvent args)
    {
        ...
        var stream = _audioSystem.PlayPvs(component.PagingSound, uid, AudioParams.Default.WithLoop(true));
        if (stream.HasValue) component.PlayingStream = stream.Value.Entity;
