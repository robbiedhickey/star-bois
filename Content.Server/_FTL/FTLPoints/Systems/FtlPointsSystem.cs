    ...
    public MapId GenerateSector(int maxStars, MapId? startingPoint, bool clear = false, bool deleteStars = false)
    {
        ...
        while (starsCreated <= maxStars)
        {
            ...
            foreach (var origin in toIter)
            {
                ...
                {
                    var prototype = _prototypeManager.Index<FtlPointPrototype>(_prototypeManager.Index<WeightedRandomPrototype>(_configurationManager.GetCVar(CCVarsFTL.StarmapRandomPrototypeId)).Pick());
