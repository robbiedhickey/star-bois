...
public sealed class ShipWeaponsSystem : SharedShipWeaponsSystem
{
            ...
            foreach (var entity in outputs)
            {
                ...
                switch (args.Action)
                {
                    case ShipWeaponAction.Fire:
                        ...
                        _gunSystem.AttemptShoot(entity, new Entity<GunComponent>(entity, gunComponent), shipWeaponComponent.Target);
                        break;
                    ...
                    case ShipWeaponAction.Chamber:
                        // CycleCartridge removed upstream — autofire handles cycling
                        break;
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var entity, out var weaponComponent))
        {
            ...
            {
                _gunSystem.AttemptShoot(entity, new Entity<GunComponent>(entity, gunComponent), weaponComponent.Target);
            }
        }
    }
    ...
    {
        _userInterface.TryOpenUi(uid, ShipWeaponTargetingUiKey.Key, args.User);
        UpdateUserInterface(uid, component);
    }
}
