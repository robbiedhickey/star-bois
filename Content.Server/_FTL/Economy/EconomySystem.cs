...
public sealed class EconomySystem : SharedEconomySystem
{
        ...
        switch (args.Action)
        {
            case IdAtmUiAction.Withdrawal:
                ...
                if (creditCardComponent.Balance >= amount)
                {
                    ...
                    var money = _stackSystem.SpawnAtPosition(amount, _prototypeManager.Index<StackPrototype>("Credit"), Transform(uid).Coordinates);
    ...
    #region Outpost ATM
    // Outpost ATM uses Frontier's StationBankAccountComponent — stubbed until we add our own economy layer.
    ...
    private void OnOutpostATMGetVerbs(EntityUid uid, OutpostATMComponent component, GetVerbsEvent<ActivationVerb> args) { }
